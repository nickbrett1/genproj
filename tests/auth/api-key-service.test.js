...
import * as db from "../../src/auth/db.js";

vi.mock("../../src/auth/db.js", () => ({
  getApiKeysDb: vi.fn(),
...
describe("Rate Limiting in ApiKeyService", () => {
  ...
  it("throws error when rate limit is exceeded", async () => {
    ...
    db.getApiKeysFirstResult.mockResolvedValue({
      ...
      rate_limit_reset_at: futureDate
        .toISOString()
        .replace("T", " ")
        .replace("Z", ""),
    });

    await expect(service.validateKey("pat_123456")).rejects.toThrow(
      "Rate limit exceeded",
    );
  });
