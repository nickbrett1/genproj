...
describe("SonarCloudAPIService", () => {
  ...
  it("validates token and retrieves resources", async () => {
    ...
    service.makeRequest.mockResolvedValue({
  ...
  it("creates, updates, and deletes projects", async () => {
    ...
    vi.spyOn(service, "makeRequest")
      .mockResolvedValueOnce({
        json: vi.fn().mockResolvedValue(projectPayload),
      })
      .mockResolvedValueOnce({
        json: vi.fn().mockResolvedValue(genericPayload),
      })
      .mockResolvedValueOnce({})
      .mockResolvedValueOnce({
        json: vi.fn().mockResolvedValue(genericPayload),
    ...
    expect(
      await service.updateProject("org_project", { name: "Updated" }),
    ).toEqual(genericPayload);
  ...
  it("manages quality gates and webhooks", async () => {
    ...
    vi.spyOn(service, "makeRequest")
      ...
      .mockResolvedValueOnce({})
      .mockResolvedValueOnce({
        json: vi.fn().mockResolvedValue({ qualityGate: {} }),
      })
      .mockResolvedValueOnce({
        json: vi.fn().mockResolvedValue(webhookPayload),
      })
      .mockResolvedValueOnce({
        json: vi.fn().mockResolvedValue({ webhooks: [] }),
      })
      .mockResolvedValueOnce({});

    expect(await service.listQualityGates()).toEqual(gatesPayload);
    await service.associateQualityGate("project", "1");
    expect(await service.getProjectQualityGate("project")).toEqual({
      qualityGate: {},
    });
  ...
  it("retrieves analysis status, triggers analysis, and fetches metrics", async () => {
    ...
    vi.spyOn(service, "makeRequest")
      .mockResolvedValueOnce({
        json: vi.fn().mockResolvedValue(activityPayload),
      })
      .mockResolvedValueOnce({
        json: vi.fn().mockResolvedValue(triggerPayload),
      })
      .mockResolvedValueOnce({
        json: vi.fn().mockResolvedValue(metricsPayload),
      });

    expect(await service.getAnalysisStatus("project")).toEqual(activityPayload);
    expect(await service.triggerAnalysis("project", "main")).toEqual(
      triggerPayload,
    );
    expect(await service.getProjectMetrics("project", ["coverage"])).toEqual(
      metricsPayload,
    );
  });
});
