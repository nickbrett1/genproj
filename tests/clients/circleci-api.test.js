...
describe("CircleCIAPIService", () => {
  ...
  it("makes authenticated requests and handles errors", async () => {
    ...
    await expect(service.makeRequest("/ping")).resolves.toEqual(
      successResponse,
    );
  ...
  it("retries followProject with backoff when CircleCI 404s a freshly-created repo", async () => {
    ...
    try {
      ...
      const followPromise = retryingService.followProject(
        "github",
        "org",
        "example",
      );
  ...
  it("does not retry followProject on non-404 errors", async () => {
    ...
    await expect(
      retryingService.followProject("github", "org", "example"),
    ).rejects.toThrow("CircleCI API error: 401 Unauthorized");
  ...
  it("updates project settings (e.g. the default branch)", async () => {
    const settingsJson = vi
      .fn()
      .mockResolvedValue({ vcs: { default_branch: "main" } });
  ...
  it("supports pipeline operations", async () => {
    ...
    expect(
      await service.listPipelines("github", "org", "repo", "token"),
    ).toEqual({ items: [] });
  ...
  it("manages environment variables", async () => {
    ...
    expect(
      await service.listEnvironmentVariables("github", "org", "repo"),
    ).toEqual([
      {
        name: "VAR",
      },
    ...
    expect(
      await service.createEnvironmentVariable(
        "github",
        "org",
        "repo",
        "VAR",
