...
describe("DopplerAPIService", () => {
  ...
  it("supports basic CRUD operations for projects", async () => {
    ...
    expect(
      await service.updateProject("project", { description: "new" }),
    ).toEqual({ ok: true });
  ...
  it("manages environments and secrets", async () => {
    ...
    const jsonSecret = vi
      .fn()
      .mockResolvedValue({ name: "TOKEN", value: "123" });
    ...
    expect(await service.createEnvironment("proj", "Dev", "dev")).toMatchObject(
      { slug: "dev" },
    );
    expect(await service.listSecrets("proj", "dev")).toEqual([
      { name: "TOKEN" },
    ]);
    expect(
  ...
  it("retrieves activity logs and validates tokens", async () => {
    ...
    expect(service.makeRequest).toHaveBeenCalledWith(
      "/projects/proj/activity?page=3&per_page=5",
    );
