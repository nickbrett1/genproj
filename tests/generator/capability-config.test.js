...
import { CapabilityConfigurationService } from "../../src/generator/capability-config.js";
...
describe("CapabilityConfigurationService", () => {
  ...
  it("validates capability selections with dependency resolution", () => {
    ...
    expect(result.warnings).toContain(
      "Some dependencies will be automatically added",
    );
  });
  ...
  it("detects conflicting capabilities", () => {
    ...
    const result = custom.validateCapabilitySelection([
      "sveltekit",
      "conflicting",
    ]);
    expect(result.isValid).toBe(false);
  ...
  it("exposes configuration schema and validates configuration", () => {
    expect(service.getCapabilityConfigurationSchema("circleci")).toMatchObject({
      cache: true,
    });
    ...
    expect(validation.warnings).toContain(
      "Unknown configuration option: unknownOption",
    );
