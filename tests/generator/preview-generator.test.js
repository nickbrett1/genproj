...
} from "../../src/generator/preview-generator.js";

// Mock TemplateEngine to control its behavior in tests
vi.mock("../../src/generator/file-generator.js", async (importOriginal) => {
  ...
  class MockTemplateEngine {
    constructor() {
      ...
      this.generateFile = vi.fn((templateId, data) => {
        ...
        if (
          templateId.startsWith("devcontainer-") &&
          templateId.endsWith("-json")
        ) {
  ...
  return {
    ...
    getDevcontainerJsonExtras: vi.fn(() => ({
      devcontainerMounts:
        '"source=tailscale,target=/var/lib/tailscale,type=volume"',
...
// The real implementation imports capabilities from ../../src/catalog/index.js
...
vi.mock("../../src/catalog/index.js", () => ({
  capabilities: [
    ...
    },
    {
      ...
      templates: [
        { templateId: "another-template", filePath: "another/file.txt" },
      ],
    },
    {
    ...
    },
    {
      ...
      name: "SvelteKit",
      templates: [
        { templateId: "svelte-config-js", filePath: "svelte.config.js" },
      ],
    },
    {
...
vi.mock("../../src/generator/capability-resolver.js", () => ({
...
describe("generatePreview", () => {
  it("creates preview data with non-devcontainer files", async () => {
    const projectConfig = {
      name: "TestProject",
      configuration: { another: { enabled: true } },
    };
    const preview = await generatePreview(projectConfig, ["another"]);
    const anotherFolder = preview.files.find(
      (f) => f.name === "another" && f.type === "folder",
    );
    expect(anotherFolder).toBeDefined();
    const anotherFile = anotherFolder.children.find(
      (f) => f.path === "another/file.txt",
    );
  ...
  it("merges multiple devcontainer files correctly", async () => {
    ...
    const dockerfile = devcontainerFolder.children.find(
      (f) => f.name === "Dockerfile",
    );
    ...
    expect(devcontainerFolder.children.length).toBe(7); // devcontainer.json, Dockerfile, .zshrc, .p10k.zsh, .tmux.conf, post-create-setup.sh, post-start-setup.sh

    // Check the merged devcontainer.json content
    const mergedJson = JSON.parse(devcontainerJsonFile.content);
    expect(mergedJson.features).toHaveProperty(
      "feature-devcontainer-node-json",
    );
    expect(mergedJson.features).toHaveProperty(
      "feature-devcontainer-java-json",
    );
    expect(mergedJson.customizations.vscode.extensions).toContain(
      "ext-devcontainer-node-json",
    );
    expect(mergedJson.customizations.vscode.extensions).toContain(
      "ext-devcontainer-java-json",
    );
  });
  ...
  it("generates cloudflare files when capability present", async () => {
    ...
    const scriptsFolder = preview.files.find(
      (f) => f.name === "scripts" && f.type === "folder",
    );
    expect(scriptsFolder).toBeDefined();

    const loginScript = scriptsFolder.children.find(
      (f) => f.name === "cloud_login.sh",
    );
    expect(loginScript).toBeDefined();

    const setupScript = scriptsFolder.children.find(
      (f) => f.name === "setup-wrangler-config.sh",
    );
    expect(setupScript).toBeDefined();

    const wranglerTemplate = preview.files.find(
      (f) => f.name === "wrangler.template.jsonc",
    );
  ...
  it("generates package.json with correct wrangler version when cloudflare-wrangler and devcontainer-node are present", async () => {
    ...
    const packageJsonFile = preview.files.find(
  ...
  it("generates sveltekit specific files", async () => {
    ...
    const svelteConfig = preview.files.find(
      (f) => f.name === "svelte.config.js",
    );
  ...
  it("generates .vscode/settings.json for python", async () => {
    ...
    const vscodeFolder = preview.files.find(
      (f) => f.name === ".vscode" && f.type === "folder",
    );
    expect(vscodeFolder).toBeDefined();

    const settingsFile = vscodeFolder.children.find(
  ...
  it("generates .vscode/extensions.json in preview recommending tmux-integrated", async () => {
    ...
    const extensionsFile = vscodeFolder.children.find(
      (f) => f.name === "extensions.json",
    );
  ...
  it("handles errors during preview generation", async () => {
    ...
    const { resolveDependencies } =
      await import("../../src/generator/capability-resolver.js");
...
describe("organizeFilesIntoFolders", () => {
  it("should organize flat file list into tree", () => {
    ...
    const utilitiesFolder = sourceFolder.children.find(
      (f) => f.name === "utils",
    );
