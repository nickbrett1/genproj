...
describe("capability-template-utils", () => {
  describe("getCodingAgentsTemplateData", () => {
    it("should generate config for sonarcloud and circleci", () => {
      ...
      expect(data.githubMcpConfig).toContain(
  ...
  describe("getGooseMcpConfig", () => {
    ...
    it("should emit the auth-off mcphub-dev dev-group block as the default toolset", () => {
      ...
      expect(data.mcphubDevGooseConfig).toContain(
    ...
    it("should keep the xcode-native local exception block without env refs", () => {
      ...
      expect(data.xcodeNativeGooseConfig).toContain(
    ...
    it("should keep the remote svelte MCP block when the sveltekit capability is selected", () => {
      ...
      expect(data.svelteGooseConfig).toContain(
  ...
  circleci:
    ...
    timeout: 300`;
      expect(() => assertNoGooseEnvVarReferences(block, "circleci")).toThrow(
        /env var reference/,
      );
    });
  ...
  sonarqube:
    ...
    timeout: 300`;
      expect(() => assertNoGooseEnvVarReferences(block, "sonarqube")).toThrow(
        /env var reference/,
      );
    });
  ...
  circleci:
    ...
    timeout: 300`;
      expect(() =>
        assertNoGooseEnvVarReferences(clean, "circleci"),
      ).not.toThrow();
      expect(() => assertNoGooseEnvVarReferences("", "circleci")).not.toThrow();
      expect(() =>
        assertNoGooseEnvVarReferences(undefined, "circleci"),
      ).not.toThrow();
    });
  });
  ...
  describe("getCircleCiTemplateData", () => {
    ...
    it("should configure gitguardian orb and job", () => {
      const context = {
        ...
        configuration: {
          circleci: { context: { enabled: true, name: "ctx" } },
        },
      };
      const data = getCapabilityTemplateData("circleci", context);
    ...
    it("should configure wrangler setup with doppler", () => {
      ...
      const data = getCapabilityTemplateData("circleci", context);
      expect(data.preBuildSteps).toContain(
    ...
    it("should use a BuildKit registry cache for the docker-publish job (genproj-docker-build-speedup)", () => {
      ...
      expect(job).toContain(
        "CACHE_REF: ghcr.io/nickbrett1/cache-app:buildcache",
      );
  ...
  describe("resolveDopplerTarget", () => {
    it("defaults to the shared common project", () => {
      expect(
        resolveDopplerTarget({ projectName: "mailroom", configuration: {} }),
      ).toEqual({
        project: "common",
        config: "dev",
        strategy: "common",
