...
const {
  ...
  const mockSonar = {
    ...
    listQualityGates: vi
      .fn()
      .mockResolvedValue([{ id: "gate", isDefault: true }]),
...
import { ExternalServiceIntegrationService } from "../../src/generator/external-service-integration.js";
...
describe("ExternalServiceIntegrationService", () => {
  ...
  it("creates a dedicated Doppler project when projectStrategy=new", async () => {
    ...
    const doppler = await service.integrateDoppler({
      ...
      configuration: {
        doppler: {
          projectStrategy: "new",
          secrets: context.configuration.doppler.secrets,
        },
      },
    });
    expect(doppler.success).toBe(true);
    ...
    expect(mockDoppler.createProject).toHaveBeenCalledWith(
      "Demo",
  ...
  it("handles SonarCloud integration with quality gate and webhook", async () => {
    ...
    expect(mockSonar.associateQualityGate).toHaveBeenCalledWith(
      "org_repo",
      "gate",
    );
