...
import {
  ...
  logError,
} from "../../src/generator/genproj-errors.js";
...
describe("genproj error utilities", () => {
  ...
  describe("error classes", () => {
    ...
    it("sets subclass-specific fields", () => {
      ...
      const external = new ExternalServiceError(
        "broken",
        "service",
        new Error("boom"),
      );
  ...
  describe("withErrorHandling", () => {
    it("returns handler result when successful", async () => {
      ...
      await expect(handler({ request: { method: "GET" } })).resolves.toEqual({
        status: 200,
      });
    });
  ...
  describe("validateRequest", () => {
    ...
    it("throws on invalid type, length, pattern, and enum", () => {
      expect(() =>
        validateRequest(
          { name: 123, email: "bad", status: "disabled" },
          schema,
        ),
    ...
    it("passes with valid payload", () => {
      expect(() =>
        validateRequest(
          { name: "alex", email: "user@example.com", status: "active" },
          schema,
        ),
      ).not.toThrow();
    });
  ...
  describe("requireAuthentication", () => {
    ...
    it("throws when service auth missing", () => {
      ...
      expect(() =>
        requireAuthentication(authState, ["github", "doppler"]),
  ...
  describe("handleExternalServiceError", () => {
    ...
    it("translates rate limit errors", () => {
      const error = {
        ...baseError,
        status: 429,
        headers: new Map([["Retry-After", 90]]),
      };
      expect(() => handleExternalServiceError(error, "github")).toThrowError(
        RateLimitError,
      );
    });
    ...
    it("translates auth, not found, server, and default errors", () => {
      ...
      expect(() =>
        handleExternalServiceError(baseError, "github"),
      ).toThrowError(ExternalServiceError);
    });
  });
  ...
  describe("getUserFriendlyMessage", () => {
    it("returns custom messages for known errors", () => {
      expect(getUserFriendlyMessage(new ValidationError("bad", "field"))).toBe(
        "bad",
      );
      expect(
        getUserFriendlyMessage(new AuthenticationError("auth", "github")),
      ).toBe("Please sign in to continue");
      expect(
        getUserFriendlyMessage(
          new AuthorizationError("nope", ["GitHub", "Doppler"]),
        ),
      ).toBe("Additional authentication required: GitHub, Doppler");
      expect(
        getUserFriendlyMessage(new NotFoundError("missing", "resource")),
      ).toBe("The requested resource was not found");
      expect(
        getUserFriendlyMessage(new ExternalServiceError("fail", "service")),
      ).toBe(
        "External service is temporarily unavailable. Please try again later.",
      );
      expect(
        getUserFriendlyMessage(new RateLimitError("slow", "service", 10)),
