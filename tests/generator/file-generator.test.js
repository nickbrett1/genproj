...
# The installers below download release tarballs to the current working
...
# Put the locally installed tools on PATH for the non-root user
...
describe("TemplateEngine", () => {
  ...
  it("initializes successfully and loads template strings", () => {
    ...
    expect(typeof engine.templates.get("devcontainer-node-json")).toBe(
      "string",
    );
  });
  ...
  it("replaces variables in template string", () => {
    const result = engine.compileTemplate(
      "Hello {{name}} and {{nested.prop}}",
      {
        name: "world",
        nested: { prop: "value" },
      },
  ...
  it("generates files and handles missing templates", () => {
    ...
    expect(() => engine.generateFile("missing", {})).toThrow(
      "Template not found",
    );
  });
  ...
  it("generates doppler.yaml pointing at the shared common project by default", () => {
    ...
    expect(content).toBe(
      dopplerYamlTemplateContent.replace("{{dopplerProject}}", "common"),
    );
  });
  ...
  it("generates doppler.yaml pointing at a dedicated project when projectStrategy=new", () => {
    ...
    expect(content).toBe(
      dopplerYamlTemplateContent.replace("{{dopplerProject}}", "test-project"),
    );
  });

  it("generates vscode-tasks.json correctly", () => {
    const content = engine.generateFile("vscode-tasks-json", {
      projectName: "test-project",
    });
    expect(content).toBe(
      vscodeTasksJsonTemplateContent.replace("{{projectName}}", "test-project"),
    );
  });
  ...
  it("generates multiple files collecting errors", () => {
    ...
    const success = results.find(
      (entry) => entry.templateId === "devcontainer-java-dockerfile",
    );
  ...
  it("README documents doppler env-var precedence and shared common project by default", async () => {
    ...
    expect(readme.content).not.toContain(
      "doppler projects create test-project",
    );
  ...
  it("README documents dedicated project provisioning when projectStrategy=new", async () => {
    ...
    expect(readme).toBeDefined();
    expect(readme.content).toContain(
      "doppler setup --project test-project --config dev",
    );
    ...
    expect(readme.content).toContain(
      "doppler configs create dev --project test-project",
    );
  });
  ...
  it("should include agent rules files under .agents/.rules in generated project files", async () => {
    ...
    const gitGuidelines = files.find(
      (f) => f.filePath === ".agents/.rules/git_guidelines.md",
    );
    ...
    expect(gitGuidelines.content).toContain(
