...
describe("Cloudflare Wrangler File Generation", () => {
  ...
  it("generates wrangler.jsonc and cloud_login.sh when only Cloudflare is selected", async () => {
    ...
    const cloudLogin = files.find(
      (f) => f.filePath === "scripts/cloud_login.sh",
    );
    ...
    expect(packageJsonContent.scripts).toHaveProperty(
      "build",
    ...
    const wranglerTemplate = files.find(
      (f) => f.filePath === "wrangler.template.jsonc",
    );
  ...
  it("runs the doppler block before the tailscale block in cloud_login.sh", async () => {
    ...
    expect(cloudLogin.content.indexOf("# Doppler login/setup")).toBeGreaterThan(
      -1,
    );
  ...
  it("generates wrangler.template.jsonc and setup scripts when Cloudflare and Doppler are selected", async () => {
    ...
    // Check that compatibility_date is set to today's date
    const today = new Date().toISOString().split("T")[0];
    expect(wranglerTemplate.content).toContain(
    ...
    const setupScript = files.find(
      (f) => f.filePath === "scripts/setup-wrangler-config.sh",
    );
    ...
    expect(cloudLogin.content).toContain(
      "doppler setup --no-interactive --project common",
    );
  ...
  it("generates Cargo.toml, src/lib.rs and sets up wrangler.jsonc build configuration when workerType is rust", async () => {
    ...
    expect(cargoToml.content).toContain(
