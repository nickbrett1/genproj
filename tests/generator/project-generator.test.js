...
import { CircleCIAPIService } from "../../src/clients/circleci-api.js";
import { DopplerAPIService } from "../../src/clients/doppler-api.js";
import { SonarCloudAPIService } from "../../src/clients/sonarcloud-api.js";

vi.mock("../../src/clients/github-api.js");
vi.mock("../../src/clients/circleci-api.js");
vi.mock("../../src/clients/buildkite-api.js");
...
describe("ProjectGeneratorService", () => {
  ...
  describe("generateProject", () => {
    ...
    it("should successfully generate a project", async () => {
      ...
      service.configureExternalServices = vi
        .fn()
        .mockResolvedValue(externalServices);
  ...
  describe("createGitHubRepository", () => {
    ...
    it("should create a GitHub repository with a generated description", async () => {
      ...
      service.services.github.createRepository.mockResolvedValue(
        mockRepository,
      );
    ...
    it("should recover and return existing repository if overwrite is true", async () => {
      ...
      service.services.github.getUserInfo = vi
        .fn()
        .mockResolvedValue({ login: "owner" });
      const existingRepo = {
        name: "test-project",
      ...
      service.services.github.getRepository = vi
        .fn()
        .mockResolvedValue(existingRepo);
      ...
      expect(service.services.github.getRepository).toHaveBeenCalledWith(
        "owner",
        "test-project",
      );
    });
  ...
  describe("commitFilesToRepository", () => {
    ...
    it("should commit files to the repository", async () => {
      await service.commitFilesToRepository(
        repository,
        generatedFiles,
        context,
      );
  ...
  describe("configureExternalServices", () => {
    ...
    it("should configure all selected services including dependabot", async () => {
      ...
      service.services.doppler.createEnvironment.mockResolvedValue({
        success: true,
      });
      service.services.sonarcloud.createProject.mockResolvedValue({
        success: true,
      });
      service.services.sonarcloud.listQualityGates.mockResolvedValue([
        { id: "1", isDefault: true },
      ]);
      service.services.sonarcloud.associateQualityGate.mockResolvedValue({
        success: true,
      });
      ...
      expect(results.circleci.pipeline).toEqual({
        id: "pipeline-1",
        number: 1,
      });
    ...
    it("should not fail CircleCI integration when the first pipeline cannot be triggered", async () => {
      ...
      service.services.circleci.triggerPipeline.mockRejectedValue(
    ...
    it("should provision a Buildkite pipeline and register its webhook", async () => {
      ...
      service.services.buildkite.triggerBuild.mockResolvedValue({
        number: 1,
        web_url: "u",
      });
    ...
    it("should not fail generation when the first Buildkite build cannot be triggered", async () => {
      ...
      service.services.buildkite.registerWebhook.mockResolvedValue(true);
      service.services.buildkite.triggerBuild.mockRejectedValue(
    ...
    it("should handle failures gracefully", async () => {
      ...
      expect(
        service.services.circleci.updateProjectSettings,
      ).not.toHaveBeenCalled();
    ...
    it("records webhookVerified true when CircleCI installed its push webhook", async () => {
      ...
      expect(service.services.github.listWebhooks).toHaveBeenCalledWith(
        "owner",
        "repo",
      );
    });
    ...
    it("does not fail generation when follow 404s on a fresh repo (GitHub App sync race)", async () => {
      ...
      expect(results.circleci.error).toContain(
        "has not indexed the brand-new repository",
      );
  ...
  describe("validateAllTokens", () => {
    ...
    it("should handle validation failures", async () => {
      service.services.github.validateToken.mockRejectedValue(
  ...
  describe("commitFilesToRepository merge semantics (round-4)", () => {
    ...
    it("merges capability contributions into a diverged devcontainer.json without clobbering manual edits", async () => {
      ...
      const committed =
        service.services.github.createMultipleFiles.mock.calls[0][2][0];
      ...
      // feature merged in
      expect(
        merged.features["ghcr.io/devcontainers/features/node:1"],
      ).toBeDefined();
      expect(
        merged.features["ghcr.io/devcontainers/features/python:1"].version,
      ).toBe("3.12");
      // project-owned keys untouched
      expect(merged.postCreateCommand).toBe(
        "bash .devcontainer/post-create-setup.sh",
      );
    });
    ...
    it("replaces devcontainer.json entirely when explicitly resolved to overwrite", async () => {
      ...
      expect(
        service.services.github.createMultipleFiles.mock.calls[0][2][0].content,
      ).toBe(generatedDevcontainer);
    });
    ...
    it("is a no-op when the merged result matches the existing file (monotonic)", async () => {
      ...
      const alreadyMerged = mergeDevcontainerJson(
        existingDevcontainer,
