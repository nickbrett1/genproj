...
agy-dev() {
  ...
  echo "Starting Antigravity with Doppler (common + genproj)..."
  ...
  doppler run --project common --config dev -- doppler run --forward-signals --project genproj --config dev -- agy "$@"
}
...
if [[ "$PWD" == "$HOME" ]]; then
  cd /workspaces/genproj 2>/dev/null
fi
