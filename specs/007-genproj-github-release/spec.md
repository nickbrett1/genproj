...
The vocabulary is **musl-only on Linux**, and deliberately so: musl is the libc that links statically, so one Linux artifact runs on an Alpine container, a glibc host and the NAS alike. Offering the `-gnu` spellings as well would mean two non-interchangeable binaries share one `uname`, and a musl host that fetched the glibc one would pass its sha256 check — the file is intact, and simply wrong — and fail at exec instead. A label that cannot be offered cannot be picked wrongly, and the targets the fleet cannot build (no Intel Mac agent, no glibc libc) are absent for the same reason.
