...
genproj today offers two deployment systems — `cloudflare-wrangler` (Cloudflare Workers) and `google-cloud`. There is no way to express _"deploy this as a Docker container to my own host/NAS"_ as the project's deployment mechanism. The user wants to generate projects whose deployment story is:
...
- No changes to the `docker` capability itself — it stays an _internal_ dependency used by devcontainers; `docker-container` is a distinct _deployment_ capability.
  ...
  "externalServices": [
    {
      ...
      "actions": [
        {
          "type": "create",
          "description": "GHCR package is created on the first push from CI"
        },
        {
          "type": "configure",
          "description": "Create a classic PAT with the write:packages scope (fine-grained PATs cannot access GHCR) and make GHCR_USERNAME and GHCR_TOKEN available to the build: in the CircleCI context, or in the Buildkite agent's environment hook"
        }
      ]
    }
  ],
