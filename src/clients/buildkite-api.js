...
export class BuildkiteAPIService extends BaseAPIService {
  ...
  async registerWebhook(org, slug) {
    ...
    try {
      ...
      console.warn(
        `⚠️ Could not register the webhook for ${slug}: ${error.message}`,
      );
      return false;
    }
  }
  ...
  async hasWebhook(org, slug) {
    try {
      ...
      console.warn(
        `⚠️ Could not check the webhook for ${slug}: ${error.message}`,
      );
      return false;
    }
  }
  ...
  #isAlreadyExistsError(error) {
    ...
    return (
      /name/i.test(message) && /(taken|already|exists|unique)/i.test(message)
    );
  }
  ...
  #isRetryableCreateError(error) {
    ...
    if (
      /Buildkite API error: 422/.test(message) &&
      /repositor/i.test(message)
    ) {
      return true;
    }
    // Transient server/network failures (fetch rejects with a TypeError).
    return (
      /Buildkite API error: 5\d\d/.test(message) || error instanceof TypeError
    );
  }
}
