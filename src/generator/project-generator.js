...
import { GitHubAPIService } from "../clients/github-api.js";
import { CircleCIAPIService } from "../clients/circleci-api.js";
import { BuildkiteAPIService } from "../clients/buildkite-api.js";
...
export function mergeDevcontainerJson(existingContent, generatedContent) {
  ...
  const mountTarget = (mount) => {
    const match =
      typeof mount === "string" ? mount.match(/target=([^,]+)/) : null;
    return match ? match[1] : mount;
  };
  const existingMounts = Array.isArray(existing.mounts)
    ? [...existing.mounts]
    : [];
  const generatedMounts = Array.isArray(generated.mounts)
    ? generated.mounts
    : [];
  ...
  const mergedEnv = {
    ...(generated.containerEnv || {}),
    ...(existing.containerEnv || {}),
  };
...
export class ProjectGeneratorService {
    ...
    try {
      ...
      if (this.services.github && !context.registryNamespace) {
        try {
          ...
          console.warn(
            "⚠️ Could not resolve GitHub login for registry namespace:",
            error,
          );
        }
      }
      ...
      const commit = await this.commitFilesToRepository(
        repo,
        generatedFiles,
        context,
      );
      ...
      const externalServices = await this.configureExternalServices(
        context,
        repo,
        commit,
      );
      console.log(
        `✅ Configured ${Object.keys(externalServices).length} external services`,
      );
  ...
  async createGitHubRepository(context) {
    ...
    try {
      ...
      if (error.code === "REPOSITORY_EXISTS" && overwrite) {
        ...
        const existingRepo = await this.services.github.getRepository(
          user.login,
          projectName,
        );
  ...
  async #configureCircleCI(
    ...
    if (context.capabilities.includes("circleci") && this.services.circleci) {
      try {
        ...
        const circleciProject = await this.services.circleci.followProject(
          "github",
          owner,
          repo,
        );
        ...
        try {
          ...
          console.log(
            `✅ CircleCI first pipeline triggered on ${defaultBranch}`,
          );
        } catch (pipelineError) {
          console.warn(
        ...
        results.circleci = {
          ...
          pipeline: pipeline
            ? { id: pipeline.id, number: pipeline.number }
            : undefined,
          webhookVerified,
        };
        console.log("✅ CircleCI configured successfully");
      } catch (error) {
        const clearError = await this.#describeCircleCISetupError(
          error,
          owner,
          repo,
        );
  ...
  async #ensureRequiredStatusCheck(owner, repo, branch, context) {
    ...
    try {
      const existing = await this.services.github
        .makeRequest(protectionPath)
        .catch(() => null);
      ...
      if (existing) {
        ...
        await this.services.github.makeRequest(
          `${protectionPath}/required_status_checks`,
          {
            method: "PATCH",
            body: JSON.stringify({
              strict: required.strict !== false,
              contexts: [...contexts],
            }),
          },
        );
  ...
  async #configureBuildkite(
    ...
    if (
      !context.capabilities.includes("buildkite") ||
      !this.services.buildkite
    ) {
      return;
    }
    ...
    if (capabilityConfig.provisionPipeline === false) {
      console.log(
        "ℹ️ Buildkite provisioning disabled for this project (provisionPipeline=false).",
      );
      results.buildkite = {
        success: true,
        skipped: true,
    ...
    try {
      ...
      const { pipeline, existed } =
        await this.services.buildkite.createPipeline(organization, {
          name: repo,
      ...
      const webhookRegistered = await this.services.buildkite.registerWebhook(
        organization,
        slug,
      );
      ...
      if (commitSha) {
        try {
          build = await this.services.buildkite.triggerBuild(
            organization,
            slug,
            {
              commit: commitSha,
  ...
  async #hasCircleCIWebhook(owner, repo) {
    ...
    try {
      ...
      return (
        Array.isArray(hooks) &&
        hooks.some((hook) =>
          (hook.config?.url || "").includes("circleci.com/hooks/github"),
        )
      );
    } catch (error) {
      console.warn(
        `⚠️ Could not check CircleCI webhook on ${owner}/${repo}: ${error.message}`,
      );
      return false;
    }
  }
    ...
    if (context.capabilities.includes("doppler") && this.services.doppler) {
      try {
        ...
        if (strategy === "new") {
          ...
          await this.services.doppler.createEnvironment(
            dopplerProject.slug,
            "Development",
            "dev",
          );
   ...
   */
  // `capabilities` is unused, exactly as in ftn — kept so callers (and the
  // ported tests) keep the same signature.
  ...
  async checkConflicts(context) {
    ...
    const exists = await this.services.github.repositoryExists(
      user.login,
      projectName,
    );
    if (!exists) {
