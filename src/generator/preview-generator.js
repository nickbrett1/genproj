...
function createMergedDevelopmentContainerJson(
  ...
  const baseJsonContent = templateEngine.generateFile(
    `devcontainer-${baseId.split("-")[1]}-json`,
    {
      ...projectConfig,
 ...
 */
function createDevelopmentContainerShellFiles(
  templateEngine,
  projectConfig,
  allCapabilities,
) {
  ...
  const postCreateContent = templateEngine.generateFile(
    "devcontainer-post-create-setup-sh",
    {
      ...
      gitGithubAuthSetup: allCapabilities.includes("doppler")
        ? GIT_GITHUB_AUTH_SETUP_SCRIPT
        : "",
...
async function generateDevelopmentContainerArtifacts(
  ...
  files.push(
    ...
    ...createDevelopmentContainerShellFiles(
      templateEngine,
      projectConfig,
      allCapabilities,
    ),
...
function generateSingleTemplateFile(
  ...
  try {
    ...
    if (
      ...
      otherCapabilities.includes("cloudflare-wrangler")
    ) {
...
async function generateCloudDeploymentFiles(
  templateEngine,
  projectConfig,
  allCapabilities,
  files,
  ...
  const hasDevcontainer = allCapabilities.some((c) =>
  ...
  const cloudLoginContent = templateEngine.generateFile(
    "scripts-cloud-login-sh",
    {
      ...projectConfig,
      tailscaleLogin,
      dopplerLogin,
...
function generateGitignoreFile(templateEngine, projectConfig, allCapabilities) {
  const hasDoppler = allCapabilities.includes("doppler");
  const hasWrangler = allCapabilities.includes("cloudflare-wrangler");
  const hasPython = allCapabilities.some((c) =>
    c.startsWith("devcontainer-python"),
  );
  const hasJava = allCapabilities.some((c) =>
...
function generateVscodeSettingsPreview(
  templateEngine,
  projectConfig,
  allCapabilities,
) {
...
async function generatePreviewFiles(projectConfig, executionOrder) {
  ...
  const otherCapabilities = executionOrder.filter(
  ...
  await generateCloudDeploymentFiles(
    templateEngine,
    projectConfig,
    executionOrder,
    files,
  ...
  const packageJsonFile = generatePackageJsonFile(
    templateEngine,
    projectConfig,
    executionOrder,
  );
  ...
  const rustWorkerLibraryFile = generateRustWorkerLibraryFile(
    projectConfig,
    executionOrder,
  );
...
function createPreviewSummary(projectConfig, resolution, files, services) {
  ...
  const authRequiredServices = services.filter(
    (service) => service.requiresAuth,
  );
  ...
  return {
    ...
    selectedCapabilities:
      resolution.resolvedCapabilities.length -
      resolution.addedDependencies.length,
