 ...
 *
 * Ported from the ftn monorepo's `webapp/src/lib/utils/capability-resolver.js`.
 * That version read from a second, stale registry (`utils/capabilities.js`)
 * whose IDs (`cloudflare`, `devcontainer`, `lighthouse`, …) no longer matched
 * the catalog, so dependency and conflict resolution silently did nothing for
 * most capabilities. It now resolves against the catalog itself.
 *
 ...
 * @property {boolean} isValid - Whether the selection is valid
...
function validateDependencies(selectedCapabilities, resolution, warnings) {
  for (const capabilityId of selectedCapabilities) {
    ...
    for (const depId of capability.dependencies ?? []) {
      if (
        !selectedCapabilities.includes(depId) &&
        !resolution.addedDependencies.includes(depId)
      ) {
...
export function getCapabilitiesRequiringAuth(selectedCapabilities) {
  return selectedCapabilities.filter(
    (capabilityId) =>
      (capabilities[capabilityId]?.authServices ?? []).length > 0,
  );
}
...
export function getRequiredAuthServices(selectedCapabilities) {
  ...
  for (const capabilityId of selectedCapabilities) {
    for (const service of capabilities[capabilityId]?.authServices ?? []) {
